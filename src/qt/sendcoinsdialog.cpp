    ...
    model(0)
{
    ...
    ui->sendButton->setIcon(QIcon());
     ...
     ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a FinerCoin address (e.g. BrXW1RKLDe8VMNwTwLwSiKuATN5M74EL85)"));
#endif
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (!model->getOptionsModel() || !model->getOptionsModel()->getCoinControlFeatures())
       { if(	ui->Anonymous->checkState())
        {
        sendstatus = model->sendCoins2(recipients);
    ...
    else
       {
        if(	ui->Anonymous->checkState())
               {
              sendstatus = model->sendCoins2(recipients, CoinControlDialog::coinControl);
        }
        else
        {
